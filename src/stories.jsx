    ...
    frames: [
      {
        ...
        art: 'android',
        tint: '#7fd97f',
        heading: 'Приложение Fit Track для Android',
        body: 'Откройте кабинет в Chrome на телефоне: внизу появится «Приложение '
          + 'Fit Track для Android» — нажмите «Установить» и разрешите установку. '
          + 'Вернитесь в кабинет — он сам откроется в приложении, входить заново '
          + 'не нужно. Чтобы приходили напоминания, в приложении откройте меню, '
          + '«Настройки» и нажмите «Включить уведомления».',
      },
      {
        id: 'steps',
        date: '26 сентября',
      ...
      },
      {
        ...
        art: 'install',
        tint: '#f5c65c',
        heading: 'Кабинет открывается по вашей ссылке',
        body: 'Тренер присылает персональную ссылку — откройте её, и вы уже '
          + 'внутри: ни пароля, ни кода. Чтобы не искать ссылку каждый раз, '
          + 'добавьте иконку на главный экран: на iPhone — кнопка «Поделиться», '
          + 'затем «На экран „Домой“». На Android лучше поставить приложение '
          + 'Fit Track — об этом соседний кадр.',
      },
      {
        id: 'plan-month',
        date: '25 сентября',
        art: 'schedule',
        tint: '#8ec0f5',
        heading: 'Программа месяца — во вкладке «Тренировки»',
        body: 'Каждая тренировка — свой блок: упражнение, число подходов и '
          + 'повторы. Упражнения, объединённые в суперсет, стоят одной '
          + 'группой — их делают подряд, без отдыха между собой. '
          + 'Проведённая тренировка помечается зелёным, а кнопка '
          + '«Посмотреть веса» открывает то занятие, где вы их записали.',
      },
      {
        id: 'workout-run',
        date: '25 сентября',
        art: 'journal',
        tint: '#7fd97f',
        heading: 'Занятие записывается прямо в зале',
        body: 'Нажмите «Начать тренировку» у нужного блока. Вписывайте вес и '
          + 'повторы, отмечайте выполненные подходы галочкой. Выберите '
          + 'длительность отдыха один раз — дальше он запускается сам после '
          + 'каждого отмеченного подхода, а таймер виден внизу экрана на '
          + 'любом упражнении. Тренер видит вашу тренировку одновременно с вами.',
      },
      {
        id: 'measure-self',
        date: '25 сентября',
        art: 'progress',
        tint: '#c9a4f0',
        heading: 'Замер можно записать самому',
        body: 'Встали на весы дома — откройте «Прогресс» и нажмите «Записать '
          + 'замер». Достаточно одного показателя: например, только веса. '
          + 'Если взвешивались раньше, поставьте тот день — замер запишется '
          + 'задним числом. График и изменения пересчитаются сразу.',
      },
      {
        id: 'nutrition-all',
        date: '25 сентября',
        art: 'nutrition',
        tint: '#f0a4a4',
        heading: 'Питание: норма и рацион из холодильника',
        body: 'Во вкладке «Питание» заполните анкету — рост, вес, возраст, '
          + 'цель, — и приложение посчитает вашу норму калорий и белка. '
          + 'Там же нажмите «Собрать рацион»: отметьте продукты, которые '
          + 'есть дома, пролистайте блюда вправо и влево — приложение соберёт '
          + 'день под вашу норму и покажет, чего не хватает докупить.',
      },
    ],
  },
];
